...
export const Login: React.FC = () => {
  ...
  const [loginResponse, setLoginResponse] = useState<CheckUserIdentity>();
  ...
  const handleOnClickLogin = useCallback(async () => {
    ...
    console.log('////start///');
    const response = await checkUserIdentity(nick, password, hiddentext);
  ...
  return (
    <Paper
      className={clsx(
        classes.cities,
        classes.outline,
        loginResponse?.authenticated ? classes.greenLight : classes.redLight
      )}
    >
      <InputWrapper>
